...
class UserAccountAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        (
            "Permissions",
            {"fields": ("is_active", "is_staff", "is_manager", "is_admin")},
        ),
...
class ProfileAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        "user__username",
        "user__email",
    )  # Use 'username' and 'email' instead of 'name'

    readonly_fields = (
        "balance",
        "date_created",
    )  # Make balance and date_created read-only
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if obj is None:
            ...
            form.base_fields["user"].queryset = UserAccount.objects.filter(

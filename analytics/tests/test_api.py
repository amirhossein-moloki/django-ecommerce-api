...
class AnalyticsAPITest(APITestCase):
    def setUp(self):
        ...
        Order.objects.create(user=self.user, status=Order.Status.PENDING, total_payable=Decimal("50.00"))
    ...
    def test_order_status_breakdown_endpoint(self):
        ...
        pending_status = next((item for item in response.data['data'] if item["status"] == "pending"), None)

...
معماری این سیستم بر پایه جداسازی کامل و امنیت طراحی شده است. بکاپ‌ها از سرورهای Production به یک **Object Storage** امن (مانند Google Cloud Storage) به‌صورت رمزنگاری‌شده ارسال می‌شوند. اپلیکیشن به هیچ‌وجه به کلیدهای بکاپ دسترسی ندارد.
...
-   **IAM Policy:** سرویس اکانت (Service Account) استفاده‌شده برای بکاپ، باید دارای نقش `Storage Object Admin` باشد تا بتواند بکاپ‌ها را بنویسد و مدیریت کند. دسترسی آن را به همین Bucket محدود کنید.
-   **Immutable Storage:** پیشنهاد می‌شود از قابلیت `Bucket Lock` در GCS برای پیاده‌سازی WORM (Write-Once, Read-Many) استفاده کنید تا از حذف تصادفی یا مخرب بکاپ‌ها جلوگیری شود.

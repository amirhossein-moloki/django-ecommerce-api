        ...
        with transaction.atomic():
            ...
            if result in [100, 201]:
                ...
                if amount_from_gateway != expected_amount:
                    ...
                    transaction_log.message = f"تراکنش ناموفق: عدم تطابق مبلغ. مبلغ مورد انتظار {expected_amount} بود، اما درگاه {amount_from_gateway} گزارش کرد."
                ...
                if error is None and order_id_from_gateway != str(order.order_id):
                    ...
                    order.save(update_fields=["payment_status", "updated"])
                    transaction_log.status = PaymentTransaction.Status.FAILED
                    transaction_log.message = f"تراکنش ناموفق: عدم تطابق شناسه سفارش. شناسه مورد انتظار {order.order_id} بود، اما درگاه {order_id_from_gateway} گزارش کرد."
                    transaction_log.save(
                        update_fields=[
                            "status",
                            "message",
                ...
                transaction_log.status = PaymentTransaction.Status.FAILED
                transaction_log.message = f"تراکنش ناموفق: {error_message} (Result code: {result})"
                transaction_log.save(
                    update_fields=[
                        "status",
                        "message",
